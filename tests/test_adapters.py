...
from adapters.chat_template_thinking import ChatTemplateThinkingAdapter
from adapters.default import DefaultAdapter
from adapters.doubao import DoubaoAdapter
from adapters.effort_only import EffortOnlyAdapter
...
from adapters.spark_x2 import SparkX2Adapter
...
class TestAdapterRegistry(unittest.TestCase):
    ...
    CASES = [
        # Spark-X2-Agent — enable_thinking (before generic spark)
        ("http://localhost:4000/v1", "spark-x2-agent", SparkX2Adapter),
        ...
        # MiniMax-M3 — thinking.type adaptive|disabled, no reasoning_effort
        ("http://localhost:4000/v1", "minimax-m3", MiniMaxAdapter),
        ("https://api.minimax.io/v1", "MiniMax-M3", MiniMaxAdapter),
        # Grok / SenseNova — reasoning_effort only
        ("http://localhost:4000/v1", "grok-4.6", EffortOnlyAdapter),
        ("http://localhost:4000/v1", "sensenova-6.8-flash-lite", EffortOnlyAdapter),
        ("http://localhost:4000/v1", "kimi-k3", EffortOnlyAdapter),
        ("http://localhost:4000/v1", "hy3", EffortOnlyAdapter),
        ("http://localhost:4000/v1", "step-3.7-flash", EffortOnlyAdapter),
        # Muse Spark — reasoning_effort only (responses-mode on LiteLLM)
        ("http://localhost:4000/v1", "muse-spark-1.3-contributor", EffortOnlyAdapter),
        ("http://localhost:4000/v1", "muse-spark-1.3", EffortOnlyAdapter),
        # Dots3-Note — chat_template_kwargs.enable_thinking
        ("http://localhost:4000/v1", "dots3-note-prev", ChatTemplateThinkingAdapter),
        ("http://localhost:4000/v1", "dots-3-note-prev", ChatTemplateThinkingAdapter),
        # LongCat — thinking.type via DefaultAdapter
        ("http://localhost:4000/v1", "longcat-2.0", DefaultAdapter),
...
class TestExtraBodyThinkMode(unittest.TestCase):
    ...
    def test_spark_x2_think_on(self):
        body = SparkX2Adapter().extra_body(True, reasoning_effort="high")
        self.assertEqual(body, {"enable_thinking": True, "reasoning_effort": "high"})

    def test_spark_x2_think_off(self):
        body = SparkX2Adapter().extra_body(False)
        self.assertEqual(body, {"enable_thinking": False})

    def test_effort_only_think_on(self):
        body = EffortOnlyAdapter().extra_body(True, reasoning_effort="high")
        self.assertEqual(body, {"reasoning_effort": "high"})

    def test_effort_only_think_off(self):
        body = EffortOnlyAdapter().extra_body(False, reasoning_effort="high")
        self.assertIsNone(body)

    def test_chat_template_thinking_on(self):
        body = ChatTemplateThinkingAdapter().extra_body(True, reasoning_effort="high")
        self.assertEqual(body, {"chat_template_kwargs": {"enable_thinking": True}})

    def test_chat_template_thinking_off(self):
        body = ChatTemplateThinkingAdapter().extra_body(False)
        self.assertEqual(body, {"chat_template_kwargs": {"enable_thinking": False}})

    def test_minimax_think_on(self):
        body = MiniMaxAdapter().extra_body(True, reasoning_effort="max")
        self.assertEqual(body, {"thinking": {"type": "adaptive"}})

    def test_minimax_think_off(self):
        body = MiniMaxAdapter().extra_body(False, reasoning_effort="max")
        self.assertEqual(body, {"thinking": {"type": "disabled"}})
...
class TestPatchRequestArgs(unittest.TestCase):
    ...
    def test_spark_patch_adds_lora_id_header(self):
        args = {"model": "spark-x", "extra_body": {"thinking": {"type": "enabled"}}}
        patched = SparkAdapter().patch_request_args(args)
        self.assertEqual(patched["extra_headers"], {"lora_id": "0"})
        # Existing extra_headers are preserved / merged
        args2 = {
            "model": "xopdeepseekv4pro0813",
            "extra_headers": {"X-Custom": "1"},
        }
        patched2 = SparkAdapter().patch_request_args(args2)
        self.assertEqual(patched2["extra_headers"]["lora_id"], "0")
        self.assertEqual(patched2["extra_headers"]["X-Custom"], "1")
...
class TestFullFlowSimulation(unittest.TestCase):
    ...
    def test_spark_x2_think_on_full(self):
        args = self._simulate_request_build(
            "http://localhost:4000/v1", "spark-x2-agent", True
        )
        self.assertEqual(args["extra_body"], {"enable_thinking": True})

    def test_grok_think_on_without_effort(self):
        args = self._simulate_request_build(
            "http://localhost:4000/v1", "grok-4.6", True
        )
        self.assertNotIn("extra_body", args)

    def test_sensenova_effort_full(self):
        adapter = get_adapter("http://localhost:4000/v1", "sensenova-6.8-flash-lite")
        body = adapter.extra_body(True, reasoning_effort="high")
        self.assertEqual(body, {"reasoning_effort": "high"})
        self.assertNotIn("thinking", body or {})

    def test_minimax_think_on_full(self):
        args = self._simulate_request_build(
            "http://localhost:4000/v1", "minimax-m3", True
        )
        self.assertEqual(args["extra_body"], {"thinking": {"type": "adaptive"}})
        self.assertNotIn("reasoning_effort", args["extra_body"])

    def test_minimax_think_off_full(self):
        args = self._simulate_request_build(
            "http://localhost:4000/v1", "minimax-m3", False
        )
        self.assertEqual(args["extra_body"], {"thinking": {"type": "disabled"}})
