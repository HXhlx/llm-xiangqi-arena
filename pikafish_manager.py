...
def _default_engine_filename() -> str:
    if os.name == "nt":
        return "pikafish-bmi2.exe"
    for name in (
        "pikafish-vnni512",
        "pikafish-avx512",
        "pikafish-avxvnni",
        ...
        if os.path.isfile(os.path.join(BASE_DIR, "pikafish", name)):
            return name
    return "pikafish-bmi2"


DEFAULT_ENGINE_FILENAME = _default_engine_filename()
DEFAULT_ENGINE_PATH = os.path.join(BASE_DIR, "pikafish", DEFAULT_ENGINE_FILENAME)
...
class PikafishEvaluator:
    ...
    async def start(self):
        ...
        # cwd = binary dir so relative EvalFile (pikafish.nnue) resolves on Win/Linux
        engine_dir = os.path.dirname(os.path.abspath(self.engine_path)) or None
    ...
    async def shutdown(self):
        """Reap the engine subprocess even if the UCI protocol already died."""
        proc = self._process
        if not proc:
            return
        self._alive = False
